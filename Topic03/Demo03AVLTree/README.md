# AVL trees and logarithmic height

An AVL tree is a binary search tree that keeps the heights of each node's two
subtrees within one of each other. This limits the longest search path even when
keys arrive in an inconvenient order. Ordered in-memory indexes and dictionaries
are typical uses when predictable logarithmic lookup/update cost is important.
...
`height()` is O(1), using the cached root height.
...
## How this implementation keeps balance (optional)

`ds.AVLTree<E>` stores a height in every private node. Its balance factor is
`height(left) - height(right)`, matching the slides. Every completed operation
leaves every factor in {-1, 0, 1}. Empty children have height zero.

`insert` and `delete` first perform BST work. As recursion returns towards the
root, `rebalance` recalculates heights and repairs any imbalance. Deletion with
two children replaces the value by its inorder successor and removes the
successor's former node. Repairs may be needed at several ancestors after deletion.

| Local imbalance | Child shape | Repair |
| --- | --- | --- |
| Left side too tall | Left child leans left or is balanced | Right rotation |
| Left side too tall | Left child leans right | Left rotation at child, then right at parent |
| Right side too tall | Right child leans right or is balanced | Left rotation |
| Right side too tall | Right child leans left | Right rotation at child, then left at parent |

A rotation changes local links while preserving the inorder sequence. It keeps
the middle subtree attached and updates the former root's height before updating
its new parent's height. A heavy child with balance factor zero is relevant during
deletion and correctly uses a single rotation.

`audit` recomputes heights from the actual links rather than trusting cached
values. It checks strict comparator order, every balance factor and every stored
height. This makes a forgotten height update detectable by the tests.

For the same number of keys, AVL imposes a stricter height-balance condition than
red-black trees. That does not mean it is universally faster: workloads, comparison
cost and update overhead matter. The course example demonstrates structure and
comparison counts, not a general performance benchmark. Java 17 has no dedicated
public `AVLTree` collection class in `java.util`.
